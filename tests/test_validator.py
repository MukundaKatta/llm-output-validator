...
import pytest
from llm_output_validator import Rule, ValidationError, validate
...
# ---------------------------------------------------------------------------
# Falsy-but-meaningful inline kwargs (regression)
# ---------------------------------------------------------------------------


def test_max_length_zero_rejects_nonempty():
    # max_length=0 means "must be empty"; a non-empty string must fail.
    result = validate("abc", max_length=0)
    assert result.ok is False
    assert result.rule_count == 1


def test_max_length_zero_accepts_empty():
    result = validate("", max_length=0)
    assert result.ok is True
    assert result.rule_count == 1


def test_min_length_zero_counts_as_rule():
    # min_length=0 always passes but the inline rule must still be registered.
    result = validate("abc", min_length=0)
    assert result.ok is True
    assert result.rule_count == 1


def test_empty_must_contain_counts_as_rule():
    # must_contain="" is trivially satisfied but is a real, intentional rule.
    result = validate("abc", must_contain="")
    assert result.ok is True
    assert result.rule_count == 1
